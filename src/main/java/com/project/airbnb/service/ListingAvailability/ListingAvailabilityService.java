...
public class ListingAvailabilityService implements IListingAvailabilityService {
    ...
    @Override
    public ListingAvailabilityResponse getListingAvailability(Long id) {
        return listingAvailabilityMapper.toListingAvailabilityResponse(listingAvailabilityRepository.findById(id)
                .orElseThrow(() -> new AppException(ErrorCode.LISTING_AVAILABILITY_NOT_EXISTED)));
    }

    @Override
    public PageResponse<List<ListingAvailabilityResponse>> fetchAvailabilityByListing(String listingId, int pageNo,
            int pageSize) {
        verifyHostOfListing(listingId);
        Listing listing = listingRepository.findById(listingId)
                .orElseThrow(() -> new AppException(ErrorCode.LISTING_NOT_EXISTED));
        Pageable pageable = PageRequest.of(pageNo - 1, pageSize);
        Page<ListingAvailability> listingAvailabilityPage = listingAvailabilityRepository.findByListing(listing,
                pageable);
        List<ListingAvailabilityResponse> availabilityResponses = listingAvailabilityPage.getContent().stream()
                .map(listingAvailabilityMapper::toListingAvailabilityResponse).toList();
    ...
    @Override
    public PageResponse<List<ListingAvailabilityResponse>> fetchAvailabilityByTime(String listingId,
            LocalDate startDate, LocalDate endDate, int pageNo, int pageSize) {
        Pageable pageable = PageRequest.of(pageNo - 1, pageSize);
        Page<ListingAvailability> listingAvailabilityPage = listingAvailabilityRepository
                .findByListingAndDateRange(listingId, startDate, endDate, pageable);
        List<ListingAvailabilityResponse> availabilityResponses = listingAvailabilityPage.getContent().stream()
                .map(listingAvailabilityMapper::toListingAvailabilityResponse).toList();
    ...
    public ListingAvailabilityResponse createListingAvailability(ListingAvailabilityCreationRequest request) {
        ...
        Listing listing = listingRepository.findById(request.getListing().getId())
                .orElseThrow(() -> new AppException(ErrorCode.LISTING_NOT_EXISTED));
    ...
    @PreAuthorize("hasRole('HOST') or hasRole('ADMIN')")
    @Transactional
    public ListingAvailabilityResponse updateListingAvailability(Long id, ListingAvailabilityUpdateRequest request) {
        ListingAvailability listingAvailability = listingAvailabilityRepository.findById(id)
                .orElseThrow(() -> new AppException(ErrorCode.LISTING_AVAILABILITY_NOT_EXISTED));
